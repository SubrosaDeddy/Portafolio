...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(message)
    {
        ...
        case WM_MOUSEMOVE: {
            ...
            if (!dxrr->canMove(dxrr->camara->posCam.x, dxrr->camara->posCam.z))
            {
                velx = -lastVelx*1.1;
                velz = -lastVelZ*1.1;
            }
            dxrr->vel = D3DXVECTOR3(velx, 0, velz);
            ...
            // Mouse move
            dxrr->izqder = (mouseData.lX / 2500.0f);
            dxrr->arriaba = -(mouseData.lY / 2500.0f);
            ...
            if (gamePad->IsConnected())
            {
                ...
                //D3DXVECTOR3(0, 0, 0)
                float velocidady = (float)gamePad->GetState().Gamepad.sThumbLY / 32767.0;
                float velocidadx = (float)gamePad->GetState().Gamepad.sThumbLX / 32767.0;

                if (velocidady > 0.19 || velocidady < -0.19) {
                    if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)
                        velocidady *= 14.5;
                    else if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) velocidady /= 3;
                    else velocidady *= 2.5;
                    if (velocidady > 0.19) dxrr->vel += D3DXVECTOR3(0, velocidady, 0);
                    else if (velocidady < -0.19) dxrr->vel += D3DXVECTOR3(0, velocidady, 0);
                }
                if (velocidadx > 0.19 || velocidadx < -0.19) {
                    if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)
                        velocidadx *= 14.5;
                    else if (gamePad->GetState().Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) velocidadx /= 3;
                    else velocidadx *= 2.5;
                    if (velocidadx > 0.19) dxrr->vel += D3DXVECTOR3(0, velocidadx, 0);
                    else if (velocidadx < -0.19) dxrr->vel += D3DXVECTOR3(0, velocidadx, 0);
                }
