...
public:
	...
	Camara(D3DXVECTOR3 eye, D3DXVECTOR3 target, D3DXVECTOR3 up, int Ancho, int Alto)
	{
		...
		//la de proyeccion
		D3DXMatrixPerspectiveFovLH( &proyeccion, D3DX_PI/4.0, ancho / alto, 0.01f, 2000.0f );
	...
	D3DXMATRIX UpdateCam(D3DXVECTOR3 vel/*float vel*/, float arriaba, float izqder)
	{
		D3DXVECTOR4 tempo;
		...
		D3DXVECTOR3* verticalV = new D3DXVECTOR3(refFront.x, 0, refFront.z);
		D3DXVECTOR3* horizontalV = new D3DXVECTOR3(refRight.x, 0, refRight.z);
		D3DXVec3Normalize(verticalV, verticalV);
		D3DXVec3Normalize(horizontalV, horizontalV);

		//posCam += refFront * vel.z/10.0;
		posCam += *verticalV * vel.z/10.0;

		//posCam += refRight * vel.x /20.0;
		posCam += *horizontalV * vel.x /20.0;
