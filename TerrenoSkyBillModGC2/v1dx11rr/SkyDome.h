...
private:
	...
	ID3D11ShaderResourceView* texMorning;
	ID3D11ShaderResourceView* texEvening;
	ID3D11ShaderResourceView* texNight;
	ID3D11SamplerState* texSampler;

	ID3D11Buffer* matrixBufferCB;
	ID3D11Buffer* timeBufferCB;
	MatrixType* matrices;
	float* timeptr;
...
public:
	SkyDome(int slices, int stacks, float radio, ID3D11Device** d3dDevice,
		ID3D11DeviceContext** d3dContext, WCHAR* morningTex, WCHAR* eveningTex, WCHAR* nightTex)
	{
		this->slices = slices;
		this->stacks = stacks;
		this->radio = radio;
		...
		LoadContent(morningTex, eveningTex, nightTex);
	}
	...
	bool LoadContent(WCHAR* morningTex, WCHAR* eveningTex, WCHAR* nightTex)
	{
		HRESULT d3dResult;
		...
		d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), morningTex, 0, 0, &texMorning, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), eveningTex, 0, 0, &texEvening, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile((*d3dDevice), nightTex, 0, 0, &texNight, 0);
		...
		d3dResult = (*d3dDevice)->CreateBuffer(&constDesc, 0, &timeBufferCB);
	...
	void Render(D3DXVECTOR3 trans, float time)
	{
		*timeptr = time;
		if (d3dContext == 0)
			return;
		...
		(*d3dContext)->PSSetShaderResources(0, 1, &texMorning);
		(*d3dContext)->PSSetShaderResources(1, 1, &texEvening);
		(*d3dContext)->PSSetShaderResources(2, 1, &texNight);
		...
		(*d3dContext)->UpdateSubresource(timeBufferCB, 0, 0, timeptr, sizeof(float), 0);

		(*d3dContext)->VSSetConstantBuffers(0, 1, &matrixBufferCB);
		(*d3dContext)->VSSetConstantBuffers(1, 1, &timeBufferCB);
