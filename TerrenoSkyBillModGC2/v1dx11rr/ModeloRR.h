...
private:
	...
	ID3D11Buffer* diffuseBuf;
	ID3D11Buffer* ambientBuf;
	ID3D11Buffer* timeBuf;
	//float* timeptr;
...
public:
	ModeloRR(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, char* ModelPath, WCHAR* colorTexturePath, WCHAR* normalTexturePath, WCHAR* specularTexturePath, float _posX, float _posZ, WCHAR* shader = L"Modelo.fx")
	{
		...
		CargaParametros(ModelPath, colorTexturePath, normalTexturePath, specularTexturePath, shader);//L"Assets/Tent-Tower/tent_diffuse.jpg"
	}
	...
	bool CargaParametros(char* ModelPath, WCHAR* diffuseTex, WCHAR* normalTex, WCHAR* specularTex, WCHAR* shader = L"Modelo.fx")
	{
		HRESULT d3dResult;
		...
		bool compileResult = CompileD3DShader(shader/*L"Modelo.fx"*/, "VS_Main", "vs_4_0", &vsBuffer);
		...
		compileResult = CompileD3DShader(shader/*L"Modelo.fx"*/, "PS_Main", "ps_4_0", &psBuffer);
		...
		//d3dResult = d3dDevice->CreateBuffer(&vertexDesc, 0, &generales);
		...
		//con la estructura de descripcion creamos el sampler
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &generales);
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &diffuseBuf);
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &ambientBuf);
		...
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &timeBuf);
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float ypos, D3DXVECTOR3 posCam, float specForce, float rot, char angle, float scale/**/, float time, D3DXVECTOR3 lightDir, D3DXVECTOR3 ambientColor, D3DXVECTOR3 diffuseColor, float FAA, float FAD)
	{
		//*timeptr = time;
		D3DXVECTOR4 aux;
		aux.x = lightDir.x;
		...
		D3DXVECTOR4 ambient;
		ambient.x = ambientColor.x;
		ambient.y = ambientColor.y;
		ambient.z = ambientColor.z;
		ambient.w = FAA;
		...
		d3dContext->UpdateSubresource(diffuseBuf, 0, 0, &diffuse, 0, 0);
		d3dContext->UpdateSubresource(ambientBuf, 0, 0, &ambient, 0, 0);
		//d3dContext->UpdateSubresource(timeBuf, 0, 0, &timeptr, 0, 0);
		d3dContext->UpdateSubresource(timeBuf, 0, 0, &timeV, 0, 0);
		//d3dContext->UpdateSubresource(cameraPosCB, 0, 0, &aux, 0, 0);
		...
		d3dContext->VSSetConstantBuffers(5, 1, &generales);

		d3dContext->VSSetConstantBuffers(6, 1, &diffuseBuf);
		d3dContext->VSSetConstantBuffers(7, 1, &ambientBuf);
		d3dContext->VSSetConstantBuffers(8, 1, &timeBuf);
